...
$project = Join-Path $projectRoot 'src\FreeGhost\FreeGhost.csproj'
$outputDir = Join-Path $projectRoot "src\FreeGhost\bin\$Configuration"
...
if ([string]::IsNullOrWhiteSpace($Version) -or
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Could not read package properties from FreeGhost.csproj.' }
...
if (-not $SkipBuild) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'FreeGhost build failed.' }
}

$pluginAssembly = Join-Path $outputDir 'FreeGhost.dll'
